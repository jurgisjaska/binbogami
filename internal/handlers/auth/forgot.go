...
func (h *Auth) forgot(c *echo.Context) error {
	...
	if err := c.Bind(request); err != nil {
		h.auditlog.Warn("forgot error: bad request", "error", err.Error())
		return c.JSON(http.StatusBadRequest, api.Error(requestError))
	}

	if err := c.Validate(request); err != nil {
		h.auditlog.Warn("forgot error: validation error", "error", err.Error())
	...
	if err != nil {
		h.auditlog.Warn("forgot error: user not found", "email", request.Email, "error", err.Error())
		return c.JSON(http.StatusNotFound, api.Errors("user not found", err.Error()))
	}
	...
	if resets != nil && len(*resets) >= passwordResetLimit {
		h.auditlog.Warn("forgot error: too many password resets", "user_id", user.Id)
